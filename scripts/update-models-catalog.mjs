...
const VENDOR_META = {
  ...
  nvidia: { developer: 'NVIDIA', country: 'USA' },
  liquid: { developer: 'LiquidAI', country: 'USA' },
}
...
function toEntry(m) {
  ...
  let label = cleanLabel(m.name ?? m.id)
  ...
  const isForcedFree = recentlyAdded.includes(m.id)
  if (isForcedFree && !label.toLowerCase().includes('free')) {
    label += ' (free)'
  }
  ...
  return {
    ...
    tier: (isForcedFree || Number(m.pricing?.prompt ?? '0') === 0) ? 'free' : 'paid',
